...
const App = function() {
	...
	this.getManager = (cb) => {
		this.storage.getConfig(config => this.storage.missingConfig(config) ? cb(null) : cb(addon(config.addonfolder)));
	};
		...
		this.getManager(manager => {
			...
			manager.listAddons(addons => {
				if (addons.length == 0) this.emit('update-addons-completed', addonsToUpdate);
					...
					const toBeInstalled = addons.filter(add => toUpdate.map(tu => tu.name).indexOf(add.name) !== -1);
		...
		this.getManager(manager => {
			...
			for (let i = 0; i < p.length; i++)
				manager.portals[p[i]].search(name, (err, addons) => {
					...
					if (completedSearches == p.length) this.emit('search-completed', results.sort((a, b) => b.downloads - a.downloads));
				});
		});
	});
};
