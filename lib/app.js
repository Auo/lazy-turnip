...
const App = function() {
	//RENDERING OF HTML, loads HTML files depending on name.
	...
	this.getManager = (cb) => {
		this.storage.getConfig(config => {
				if(this.storage.missingConfig(config)) {
					...
					return cb(addon(config.addonfolder))
				}
		})
	}
	...
	this.on('check-for-updates', () => {
		this.getManager(manager => {
			...
			manager.listAddons(addons => {
				...
				if(addons.length == 0) {
					this.emit('update-check-completed', addonsToUpdate)
				 }
				else {
					let addonsChecked = 0;
					...
					for(let i = 0; i < addons.length; i++) {
						manager.checkForAddonUpdate(addons[i], (err, versionInfo) => {
							...
							addonsChecked++
							if(addonsChecked == addons.length) {
								this.emit('update-check-completed', addonsToUpdate)
							}
						})
					}
	...
	this.on('update-addons', (toUpdate) => {
		this.getManager(manager => {
			...
			manager.listAddons(addons => {
				if(addons.length == 0) {
					this.emit('update-addons-completed', addonsToUpdate)
				 }
				else {

					const toBeInstalled = addons.filter(add => { return toUpdate.map(tu => { return tu.name }).indexOf(add.name) !== -1	})
					console.log(toBeInstalled, ' to install ')
					...
					for(let i = 0; i < toBeInstalled.length; i++) {

						manager.portals[toBeInstalled[i].portal].getAddonInfo(toBeInstalled[i], (err,info) => {
							manager.installAddon(info, (err, versionInfo) => {
								if(versionInfo.newVersionAvailable) { addonsToUpdate.push(toBeInstalled[i]) }
								addonsChecked++
								if(addonsChecked == toBeInstalled.length) {
									this.emit('update-addons-completed', addonsToUpdate)
								}
							})
						})
		...
		this.getManager(manager => {
			...
			for(let i = 0; i < p.length; i++) {
				manager.portals[p[i]].search(name, (err, addons) => {
					...
					if(completedSearches == p.length) {	this.emit('search-completed', results.sort((a,b) =>{ return b.downloads - a.downloads })) }
				})
			}
		})
	})
