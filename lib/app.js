		...
		this.getManager(manager => {
			...
			manager.listAddons(addons => {
				if (addons.length == 0) {
					...
					const getInfoAndInstall = () => manager.portals[toBeInstalled[addonsChecked].portal].getAddonInfo(toBeInstalled[addonsChecked], (err, info) => {
						manager.installAddon(info, (err, versionInfo) => {
							if (versionInfo.newVersionAvailable) { addonsToUpdate.push(toBeInstalled[addonsChecked]) }
							addonsChecked++
